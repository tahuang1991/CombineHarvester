...
if create_datacards:
  ...
  for tr in trainlist:
    for m in masslist:
      ...
      os.system("HHbblvlv --input_folder={inputdir} --output_folder={outputdir} --mass={mass} --training={train}".format(inputdir = inputdir, mass = m, train = tr, outputdir = outputdir))
    ...
    for mass in masslist:
      for channel in channellist:
        ...
        if channel == "ElEl_MuEl_MuMu":
          script.write("cd .. \n")
          script.write("combineTool.py -M T2W -i {m} -o {out_prefix}_{ch}_M{m}_combine_workspace.root --parallel 4 \n".format(ch = channel, m = mass, out_prefix = out_prefix))
          script.write("cd {m} \n".format(m = mass))
        else:
